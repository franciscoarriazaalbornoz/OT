...
async function buscar(){
  ...
  try{
    ...
    if(data.resultados.length === 1){
      // resultado único: entra directo a la pantalla simple de lavado (no a la del técnico)
      location.href = `/l/${data.resultados[0].id}`;
      return;
    }
    ...
    resultadosEl.querySelectorAll("[data-id]").forEach(el=>{
      el.addEventListener("click", ()=>{ location.href = `/l/${el.dataset.id}`; });
    });
  }catch(e){
